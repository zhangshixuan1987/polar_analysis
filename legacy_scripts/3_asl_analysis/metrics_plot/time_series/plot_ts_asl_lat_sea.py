...
def main(mode,metrics,metunit,exps,seas,dir_hist,dir_sspf,period_hist,period_sspf,out_dir):
        ...
        for exp in exps:
          ...
          if 'FISMF' in exp:
            fil1 = glob.glob(os.path.join(dir_hist,
                   '{}.{}.{}.{}.{}.csv'.format(mode,ekey[0].replace('-FISMF',''),ekey[1],sea,period_hist)))
            fil2 = glob.glob(os.path.join(dir_sspf,
                   '{}.{}.{}.{}.{}.csv'.format(mode,ekey[0],ekey[1],sea,period_sspf)))
          else:     
            fil1 = glob.glob(os.path.join(dir_hist,
                   '{}.{}.{}.{}.{}.csv'.format(mode,ekey[0],ekey[1],sea,period_hist)))
            fil2 = glob.glob(os.path.join(dir_sspf,
                   '{}.{}.{}.{}.{}.csv'.format(mode,ekey[0],ekey[1],sea,period_sspf)))
          f1  = pd.read_csv(fil1[0],index_col=False)
          f2  = pd.read_csv(fil2[0],index_col=False)
          ff  = pd.concat([f1, f2], join="inner")
          pcs = ff[met]
          
          #Extract the data
          dtmp_raw = np.array(pcs).ravel()
          ...
          del(ekey,fil1,fil2,f1,f2,ff,pcs,dtmp_raw,dtmp_flt)
     
        # plot 
        plot_time_series(mode,met,unt,sea,labels,period_hist,period_sspf,dts_raw,dts_flt,out_dir)
        del(dts_raw,dts_flt,labels)

def plot_time_series(mode,met,unt,sea,labels,period_hist,period_sspf,data,dflt,out_dir):
  #color map for markers
  title  = "{} {} Time Series ({})".format(mode.split(".")[0].upper(),met,sea)
  ylabel = '{} ({})'.format(met.upper(),unt)
  xlabel = 'Model Time (year)'
  colormap = "tab20_r"
  fontsize = 18
  #color map for highlight lines
  lncolors = ['#000000', '#377eb8', '#e41a1c', '#ff7f00', '#4daf4a',
  ...
  data = np.array(data)
  dflt = np.array(dflt)
  print("data range,shape (original) = {}-{},{}".format(data.min(),data.max(),data.shape))
  print("data range,shape (filter)   = {}-{},{}".format(dflt.min(),dflt.max(),dflt.shape))
  ...
  xtick = np.arange(0,nyear)
  xlabs = np.arange(0,nyear)+years
  
  labint = 10 #label interval
  ...
  ax.set_xticks(xtick[::10])
  ax.set_xticklabels(xlabs[::10].astype(int))
  ax.tick_params(labelsize=fontsize)

  #plt.show()
  figname = os.path.join(out_dir,'fig_{}_{}_{}_{}.pdf'.format(mode,met,sea,period)) 
  plt.savefig(figname)
...
if __name__ == "__main__":
  ...
  mode = "ASL.index"
  dir_hist = os.path.join(ts_path,"e3sm/historical")
  dir_sspf = os.path.join(ts_path,"e3sm/ssp370")
  metrics = ['lon','lat','ActCenPres','SectorPres','RelCenPres']
