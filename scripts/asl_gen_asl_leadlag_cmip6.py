...
def main(fig_path,out_path,mip,exp,relm,case_id,period,case_dict,asl_region,reg_idx,l_check_asl_region):
  for key in case_dict:
    if key == "mask":
      dmsk = case_dict[key].path
      vmsk = case_dict[key]._var
    elif key == "asl":
      dasl = case_dict[key].path
      vasl = case_dict[key]._var
    else:
      case = key 
  ...
  #load index file
  if os.path.exists(dasl):
    asl_df = pd.read_csv(dasl,index_col=False)
  else: 
    print(dasl)
    exit("asl index file not found ....")
  ...
  if not l_asl_exist: 
    exit('asl index not found in {}'.format(dasl))
  else: 
    asl = asl.assign_coords({"time": da.time})
  
  #finally perform the lead lag analysis 
  asl_lead_lag_anl(mip,exp,relm,case,case_id,out_path,fig_path,da,asl,reg_idx,time) 
...
def asl_lead_lag_anl(mip,exp,relm,case,case_id,out_path,fig_path,da,asl,reg_idx,time):
  lons    = da['longitude'][:]
  lats    = da['latitude'][:]
  ...
  #raslSD    = aslSD.rolling(time=5, center=True).mean('time')
  ...
  for tllg in leadlag: 
    ...
    pvl = xs.pearson_r_p_value(raslSD, rdanm.shift(time=tllg), dim="time",skipna=True)
    corll.append(cor)
    regll.append(reg)
    pvlll.append(pvl)
    del(cor,reg,pvl)
  ...
  save_figure_data(corll,regll,pvlll,leadlag,mip,exp,relm,case,case_id,period,var,vunt,reg_idx,out_path)
...
def save_figure_data(vcor,vreg,pval,leadlag,mip,exp,relm,case,case_id,period,var,vunt,reg_idx,out_path):
  ...
  df = xr.Dataset(
          ...
          attrs=dict(description="asl lead/lag regression maps",
  ...
  fout_name = "{}_{}_{}_{}_{}_{}_{}_leadlag_{}.nc".format(mip,exp,case,relm,case_id,var,reg_idx,period)
...
def define_asl(da,region):
  ...
  asl_sst    = wgt_areaave(da,region) 
  asl_clm    = wgt_areaave(dclm,region)
  asl_ind    = wgt_areaave(anm,region) 
  aslSD_ind  = asl_ind / asl_ind.std(dim='time')
  #instead of moving average, using a low-pass filtering 
  #rasl_ind   = asl_ind.rolling(time=5, center=True).mean('time')
  #oasl_ind   = asl_ind.rolling(time=3, center=True).mean('time')
  #raslSD_ind = aslSD_ind.rolling(time=5, center=True).mean('time')
  #oaslSD_ind = aslSD_ind.rolling(time=3, center=True).mean('time')
  rasl_ind    = low_pass(1.0/5.0,asl_ind,axis=0)   #5-month: 1/5.0
  oasl_ind    = low_pass(1.0/3.0,asl_ind,axis=0)   #3-month: 1/3.0
  raslSD_ind  = low_pass(1.0/5.0,aslSD_ind,axis=0) #5-month: 1/5.0
  oaslSD_ind  = low_pass(1.0/3.0,aslSD_ind,axis=0) #3-month: 1/3.0

  df = pd.DataFrame()
  df['aslsst_val(degC)'] = asl_sst
  df['aslsst_clm(degC)'] = asl_clm
  df['asl_idx(degC)']    = asl_ind
  df['aslSD_idx(1)']     = aslSD_ind
  df['rasl_idx(degC)']   = rasl_ind
  df['raslSD_idx(1)']    = raslSD_ind
  df['oasl_idx(degC)']   = oasl_ind
  df['oaslSD_idx(1)']    = oaslSD_ind
  df['time']              = time_str
  
  ### re-order columns
  df = df[['time','aslsst_val(degC)','aslsst_clm(degC)','asl_idx(degC)','rasl_idx(degC)','oasl_idx(degC)','aslSD_idx(1)','raslSD_idx(1)','oaslSD_idx(1)']]
  ### clean-up DataFrame
  df = df.reset_index(drop=True)
  del(clm,anm,dclm,da,asl_sst,asl_clm,asl_ind,rasl_ind,aslSD_ind,raslSD_ind,times,time_str)
  return df
...
if __name__ == "__main__":
  ...
  fig_path = os.path.join("/lcrc/group/e3sm/public_html/diagnostic_output/ac.szhang/polar_analysis/figure", "asl_analysis","lead_lag_analysis","figure")

  # region of interest (asl sector)
  asl_region   = {'west':-180., 'east':180., 'south':-90., 'north':-20.}
  ...
  for k,var in enumerate(varlist):
    ...
    print(glob.glob(os.path.join(ptmp,mip+"."+exp+".*."+relm+"*"+var+"*.nc")))
    for filePath in flst:
      if os.path.isfile(filePath):
        ...
        if os.path.isfile(index_fil):
          ...
          case_dict['asl'] = Case(index_fil, var = "asl", color="blue", label=case)
          #call fuction to generate regression
          main(fig_path,out_path,mip,exp,relm,case_id,period,case_dict,asl_region,reg_idx,l_check_asl_region)
