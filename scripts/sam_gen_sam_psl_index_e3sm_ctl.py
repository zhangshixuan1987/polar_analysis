...
def main(fig_path,out_path,mip,exp,relm,case_id,period,case_dict,sam_region,l_check_sam_region):
  ...
  ################################
  #sanity check on sam region
  ################################
  if l_check_sam_region:
    ...
    da_nmsk = slice_region(da_t, sam_region)
    da_mask = slice_region(da_mask, sam_region)
    print(da_mask)
  ...
  ################################
  #end sanity check on sam region
  ################################
  var = da.name 
  vstr = da.name.upper()
  vunt = da.units
  ...
  #plot map with sam regression on TS
  draw_sam_map(out_path,fig_path,ds[var],sam_df,colnams,sam_region,mip,exp,relm,case,case_id,period,vstr,vunt)
...
def draw_sam_map(out_path,fig_path,da,sam_df,colnams,sam_region,mip,exp,relm,case,case_id,period,var,vunt):
  lons  = da['longitude'][:]
  lats  = da['latitude'][:]
  ...
  #ssta_series  = sam_df['sam_idx(degC)']  #.reshape(len(time))
  #rssta_series = sam_df['rsam_idx(degC)'] #.reshape(len(time))
  #sam_series  = sam_df['samSD_idx(1)']   #.reshape(len(time))
  #rsam_series = sam_df['rsamSD_idx(1)']  #.reshape(len(time))
  time  = sam_df['time']
  xtime = np.linspace(1,len(time),len(time))
  ...
  for col in colnams: 
    if 'sam_idx' in col: 
      ...
      sam_exist = True
  if not sam_exist:
    exit('sam_idx not exist, please check....')
  ...
  # simultaneous
  #vreg,pval = pearsonr(rsamSD,rdanm,axis=0)
  vcor = xr.corr(rsamSD, rdanm, dim="time")
  vreg = xr.cov(rsamSD, rdanm, dim="time")/rsamSD.var(dim='time',skipna=True).values
  pval = xs.pearson_r_p_value(rsamSD, rdanm, dim="time",skipna=True)
  ...
  ax1, fill1 = draw_regression_map(vstr,vunt,lons,lats,vcor,vreg,pval,fig,sam_region,
...
def define_sam(mip,exp,relm,case,case_id,period,vstr,vunt,da,region,out_path):
  ...
  print(north_lat,south_lat)
  iplat = lats.where( (lats == north_lat ) | (lats == south_lat), drop=True)
  dsub  = da.sel(latitude=iplat).mean(("longitude"), skipna=True)
  ...
  sam_psl65S = dsub.sel(latitude=south_lat).values
  sam_psl40S = dsub.sel(latitude=north_lat).values
  sam_clm65S = dclm.sel(latitude=south_lat).values
  sam_clm40S = dclm.sel(latitude=north_lat).values
  sam_dpsl   = anm.sel(latitude=north_lat).values - anm.sel(latitude=south_lat).values
  sam_ind    = norm.sel(latitude=north_lat).values - norm.sel(latitude=south_lat).values
  ...
  for col in colnams: 
    if 'time' in col: 
      df[col] = time_str
    elif 'sam_psl65S' in col:
      df[col] = sam_psl65S
    elif 'sam_psl40S' in col:
      df[col] = sam_psl40S      
    elif 'sam_clm65S' in col: 
      df[col] = sam_clm65S
    elif 'sam_clm40S' in col:
      df[col] = sam_clm40S
    elif 'sam_dpsl' in col:
      df[col] = sam_dpsl
    elif 'sam_idx' in col:
      df[col] = sam_ind
  ...
  del(clm,anm,norm,dclm,da,sam_psl65S,sam_psl40S,sam_clm65S,sam_clm40S,sam_dpsl)
  del(iplat,lons,lats,dsub,sam_ind,rsam_ind,samSD_ind,rsamSD_ind,times,time_str)
