...
def main(fig_path,out_path,mip,exp,relm,case_id,period,pclimo,case_dict,region,regnam,season):  
  ...
  if len(data) > 1:
    ...
    for k in range(1,len(data)):
      ds = xr.concat([ds,xr.open_dataset(data[k])],'time') 
  else: 
...
if __name__ == "__main__":
  ...
  mip       = "e3sm"
  exps      = ["ssp370"]
  ekey      = ["v2_1-SORRM.0701",
               "v2_1-SORRM.0751",
               "v2_1-SORRM.0801",
               "v2_1-SORRM-FISMF.0701"]
  case_id   = 'eof_gwmsl'
  tableId   = "Amon"
  varlist   = ["ts", "tas"]
  varstrs   = ["TS", "TREFHT"]
  period    = "195001-210012" 
    ...
    for j,var in enumerate(varlist): 
      for exp in exps:
        for key in ekey:
          ...
          ftmp = '{}.{}.{}.{}.*.{}.{}.nc'.format(mip,exp,case,relm,var,period.replace('1950','2015'))
          ...
          if len(filePath) > 0 and os.path.isfile(filePath[0]):
            if case == 'v2_1-SORRM-FISMF':
              fftmp = filePath[0].replace(exp,'historical').replace('201501-210012','195001-201412')
              fftmp = fftmp.replace(case,'v2_1-SORRM')
            else:
              fftmp = filePath[0].replace(exp,'historical').replace('201501-210012','195001-201412')
            
            if not os.path.exists(fftmp):
               exist('file {} not exist'.format(fftmp))
            
            data_fil = [fftmp,filePath[0]]
