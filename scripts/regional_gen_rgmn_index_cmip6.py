...
def main(fig_path,out_path,mip,exp,relm,period,case_id,case_dict,region,l_check_region):
...
def draw_rgmn_ts(fig_path,rgmn_df,region,mip,exp,case,relm,case_id,var,vunt,period):
  ...
  vmin = min([min(rtsa_series),min(tsa_series)])
  vmax = max([max(rtsa_series),max(tsa_series)])
  ...
  vmin = min([min(rrgmn_series),min(rgmn_series)])
  vmax = max([min(rrgmn_series),min(rgmn_series)])
...
if __name__ == "__main__":
  ...
  out_path = os.path.join("/lcrc/group/e3sm/public_html/diagnostic_output/ac.szhang/polar_analysis/data","rgmn_analysis","raw_index")
  fig_path = os.path.join("/lcrc/group/e3sm/public_html/diagnostic_output/ac.szhang/polar_analysis/figure", "regional_analysis","regional_mean_ts","figure")

  # region of interest (rgmn sector)
  regions  = {'nino':   {'west':-170., 'east':-120.,  'south':-5.,  'north':5.},    
              'so':     {'west':-180., 'east':180.,   'south':-70., 'north':-50.},
              'asl':    {'west':-190., 'east':-62.,   'south':-80., 'north':-60.},
  ...
  for i,var in enumerate(varlist):
    ...
    for filePath in flst:
      if os.path.isfile(filePath):
        ...
        case_dict[case] = Case(data_fil, var=varstrs[i].upper(), color="blue", label=case)
