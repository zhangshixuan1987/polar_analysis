...
def main(fig_path,out_path,mip,exp,relm,period,case_id,case_dict,region): 
  ...
  if len(ftpl) > 1: 
    for i,ff in enumerate(ftpl):
      fname = ff.split("/")[-1]
      ystr  = int(fname.split("_")[1].split("-")[0])
      yend  = int(fname.split("_")[1].split("-")[1].replace(".nc",""))
...
#  da['month']= np.array(month)
  ...
  for var in list(da.keys()):
    ...
    if var not in ["transectNames", "startTime", "endTime", "Time", "year", "month"] and len(da[var].shape) > 1:
...
def plot_mpas_ts(fig_path,df,var,mip,exp,case,case_id,period,region):
  ...
  for i,reg in enumerate(region):
    ...
    ymin = min(df[reg])
    ymax = max(df[reg])
    ax = fig.add_subplot(int(len(region)/2+1), 2, i+1)
...
if __name__ == "__main__":
  ...
  for exp in exps:
    for relm in relms:
      ...
      ptmp = run_path.replace("%(EXPS)",rnam).replace("%(PERIOD)",'yrs1950-2014')
