...
def save_figure_data(stat,region,season,var_names,var_units,data_dict,template,outdir):
   ...
   fname = template.replace("%(metric)",stat).replace("%(region)",region).replace("%(season)",season)
...
def port4sea_plot(stat,region,seasons,data_dict,var_names,var_units,
    ...
    ax.set_title("Model Performance of Seasonal Climatology ({}, {})".format(
                 stat.upper(),region.upper()), fontsize=fontsize*1.5, pad=30)
    ...
    figname = figure_template.replace("%(metric)",stat).replace("%(region)",region).replace("%(season)","4season")
...
def paracord_plot(stat,region,season,data_dict,var_names,var_units,
                  cmip_models,test_models,highlight_models,
                  file_template,figure_template,outdir,
                  data_version=None, watermark=False):
    ...
    model_list = data_dict['model'].tolist()
    model_list_group2 = highlight_models + test_models
    models_to_highlight = ['CMIP MME','HIST ENS'] 
    ...
    title = "Mean Climate: Spatio-temporal Error ({}, {})".format(
    ...
    # Save figure as an image file
    figname = figure_template.replace("%(metric)",stat).replace("%(region)",region).replace("%(season)",season)
    figfile = os.path.join(outdir,figname)
    fig.savefig(figfile, facecolor='w', bbox_inches='tight')
...
def taylordiagram_plot(stat,region,data_dict,var_names,var_units,
                       run_type,data_name,product,period,outdir,
                       ftype,data_version=None,watermark=False):
    #process figure
    mip = data_name.split(".")[0]
    exp = data_name.split(".")[1]
    version = data_name.split(".")[2]
    ...
    #extract model information from data (adhoc)
    model_names = data_dict[stat][seasons[0]][region]['model'].tolist()
    model_names = list(map(lambda x: x.replace(product,"{}-{}".format(mip,product).upper()),model_names))
...
def main(parameter):
    ...
    test_group = ["v2_1-SORRM"]

    variables = [
        s.split("/")[-1].split("_")[0]
        for s in glob.glob(os.path.join(pmp_path,test_mip, test_exp, test_case_id, "*_{}.json".format(test_case_id)))
        if os.path.exists(s)
    ]
    for vv in ['sic','sos','tos']:
      variables.remove(vv)
    ...
    for vv in variables:
        ftest = glob.glob(
            os.path.join(pmp_path, test_mip, test_exp, test_case_id, "{}_*_{}.json".format(vv,test_case_id))
        )
        fcmip = glob.glob(
            os.path.join(pmp_path, cmip_mip, cmip_exp, cmip_case_id, "{}_*_{}.json".format(vv,cmip_case_id))
        )
        if len(ftest) > 0 and len(fcmip) > 0:
            test_list.append(ftest[0])
            cmip_list.append(fcmip[0])
            #print(ftest[0].split("/")[-1], fcmip[0].split("/")[-1])
        if run_type == "model_vs_model":
            frefr = glob.glob(
                os.path.join(pmp_path, "{}_*{}*_{}.json".format(vv, refr_mip, refr_case_id))
            )
            if len(frefr) > 0:
                refr_list.append(frefr[0])
                #print(ftest[0].split("/")[-1],frefr[0].split("/")[-1],fcmip.split("/")[-1])
            del frefr
        del (ftest, fcmip)

    #load the data and reorganize if needed
    pd.set_option('future.no_silent_downcasting', True)
    test_lib = Metrics(test_list)

    #model_vs_model, merge the reference model data into test model
    if parameter.run_type == "model_vs_model":
      refr_lib = Metrics(refr_list)
      test_lib = test_lib.merge(refr_lib)
      del(refr_lib)

    #collect cmip and model ensemble data for comparison
    cmip_lib = Metrics(cmip_list)
    cmip_models = []
    highlight_models = []
    test_models = []
    merge_dict={}
    for stat in test_lib.df_dict:
        for season in test_lib.df_dict[stat]:
            for region in test_lib.df_dict[stat][season]:
               if stat in cmip_lib.df_dict.keys():
                 if season in cmip_lib.df_dict[stat].keys():
                    if region in cmip_lib.df_dict[stat][season].keys():
                      if stat not in merge_dict.keys():
                        merge_dict[stat] = {}
                      if season not in merge_dict[stat].keys():
                        merge_dict[stat][season] = {}
                      if region not in merge_dict.keys():
                        merge_dict[stat][season][region] = {}
                        
                      dftmp = pd.DataFrame(cmip_lib.df_dict[stat][season][region])
                      iflg = 0
                      for i,model in enumerate(dftmp['model'].tolist()):
                        model_run = model+"_r1i1p1f1"
                        if dftmp['model_run'].tolist()[i] == model_run:
                          if iflg == 0:
                            df1 = dftmp[dftmp.iloc[:,2] == model_run] 
                            iflg = 1
                          else:
                            df1 = pd.concat([df1,dftmp[dftmp.iloc[:,2] == model_run]])
                          if ("e3sm" in model.lower()) and (model not in highlight_models):
                              highlight_models.append(model)

                      for model in highlight_models:
                        idxs = df1[df1.iloc[:,0] == model].index
                        for idx in idxs:
                          df1 = shift_row_to_bottom(df1, idx)
                        del(idxs)

                      for model in df1['model'].tolist():
                        if model not in cmip_models:
                          cmip_models.append(model)

                      df2 = pd.DataFrame(test_lib.df_dict[stat][season][region])
                      for model in test_group:
                        df_sub = df2.loc[df2["model"] == model]
                        df1 = pd.concat([df1,df_sub], ignore_index=True) 
                        del(df_sub)

                      for i,model in enumerate(df1["model_run"].tolist()):
                        if "v2_1-SORRM" in model: 
                          new_name = model.replace("v2_1-SORRM","HIST") 
                          idxs = df1[df1.iloc[:,2] == model ].index
                          df1.loc[idxs,"model"] = list(
                                map(lambda x: x.replace("v2_1-SORRM", new_name),df1.loc[idxs,"model"],)
                                )
                          if new_name not in test_models:
                             test_models.append(new_name)
                          del(new_name,idxs)   
                      merge_dict[stat][season][region] = df1
                      del[df1,df2,dftmp,iflg]
    ...
    var_names = test_lib.var_list
    merge_dict,var_list,var_unit_list = fill_plot_var_and_units(merge_dict,var_names)
    ...
    for metric in ["rms_xy","cor_xy","bias_xy"]:
      for region in regions:
        ...
        port4sea_plot(metric,region,seasons,data_dict,var_list,var_unit_list,
                      cmip_models,test_models,highlight_models,
    ...
    #add ensemble mean 
    for metric in ["rms_xyt","rms_y","rms_devzm","std-obs_xyt","std_xyt","std_xy_devzm","std-obs_xy_devzm"]:
      for region in regions:
        for season in ["ann"]:
          data_dict = merge_dict[metric][season][region]
          df_sub2 = merge_dict[metric][season][region]
          for i,model in enumerate(test_models):
            ...
            if i == 0: 
              ...
              del(tmp_sub1)
          data_dict.loc['CMIP MME'] = df_sub2.mean(numeric_only=True, skipna=True)
          data_dict.at['CMIP MME','model'] = 'CMIP MME'
          data_dict.loc['HIST ENS'] = df_sub1.mean(numeric_only=True, skipna=True)
          data_dict.at['HIST ENS','model'] = 'HIST ENS'
          data_dict = data_dict.fillna(value=np.nan)
          paracord_plot(metric,region,season,data_dict,var_list,var_unit_list,
                        cmip_models,test_models,highlight_models,
...
if __name__ == "__main__":
  ...
  results_dir = "/lcrc/group/e3sm/ac.szhang/acme_scratch/e3sm_project/E3SMv21_testings/paper_material/mean_climate"
  data_dir = "/lcrc/group/e3sm/ac.szhang/acme_scratch/data/pcmdi"

  parser = create_mean_climate_plot_parser()
  parameter = parser.get_parameter(argparse_vals_only=False)
  parameter.pcmdi_data_path = os.path.join(data_dir,"metrics_results","mean_climate")

  parameter.pcmdi_data_set = "cmip6.historical.v20240810"

  parameter.period = "1985-2014"
  parameter.test_product = "v2_1-SORRM"
  parameter.test_data_set = "e3sm.historical.v2_1-SORRM.v20240810"

  parameter.run_type = "model_vs_obs"
  parameter.refr_data_set = ""
  parameter.refr_data_path = ""
  parameter.regions = ['SH','90S50S','50S20S','20S20N','ocean_SH','land_SH']

  if parameter.run_type == "model_vs_model":
    parameter.refr_data_set = "e3sm.historical.v2_1-LR.v20240810"
    parameter.refr_data_path = os.path.join(results_dir,"metrics_results","mean_climate")

  parameter.output_path = os.path.join(results_dir,"southern_hemisphere","figures")
  parameter.ftype = "pdf"
