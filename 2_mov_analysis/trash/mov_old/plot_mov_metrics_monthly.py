...
def load_metrics_data_mov(path,template,mode,diags,metrics,units,seasons):
    ...
    for i,model in enumerate(out_lib["model"].tolist()):
      if model in ['IPSL-CM5A2-INCA','CAS-ESM2-0']:
...
def load_metrics_data_enso(path,template,mode,diags,metrics,units):
    ...
    #remove bad model data         
    for i,model in enumerate(out_lib["model"].tolist()):
      if model in ['MCM-UA-1-0','CMCC-CM2-SR5']:
        idxs = out_lib[out_lib.iloc[:,0] == model].index
        for metx in out_lib.columns: 
...
if __name__ == "__main__":
  ...
  results_dir = "/lcrc/group/e3sm/ac.szhang/acme_scratch/e3sm_project/E3SMv21_testings/paper_material/2_southern_hemisphere"
  ...
  parameter.data_path = os.path.join(data_dir,"fig_data","mov_analysis")
  ...
  parameter.output_path = os.path.join(results_dir,"mov","figures")
  parameter.ftype = "pdf"
