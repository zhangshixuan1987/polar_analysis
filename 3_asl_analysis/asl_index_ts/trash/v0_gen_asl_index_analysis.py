...
def main(fig_path, out_path, case_dict, asl_region, asl_min_dist, 
  ...
  for t in range(ntime):
    da_t = da.isel(time=t)
    all_lows_df  = get_lows(da_t, mask, asl_region, asl_min_dist, asl_num_peak, asl_exc_bord)
...
def asl_sector_mean(da, region, mask):
  latn = region['north'] 
  lats = region['south'] 
  lone = region['east']  
  lonw = region['west']  
  #print("average over region for ASL: latn,lats,lone,lonw=",latn,lats,lonw,lone)
  a = da.where(mask == 0).sel(latitude=slice(lats,latn),
                              longitude=slice(lonw,lone)
                              ).mean().values
  return a

def get_lows(da, mask, asl_region, min_dist, num_peak, exclue_border):
  '''
...
if __name__ == "__main__":
  ...
  cases     = ["NOAA_20C","ERA5_HRES"]
  labels    = ["NOAA_20C","ERA5_HRES"]
  ...
  for i,case in enumerate(cases):
    if case in ["NOAA_20C"]:    
      data_fil = os.path.join(run_path,case,"PSL_185301_201512.nc")
      ...
      data_fil = os.path.join(run_path,case,"psl_197901_201912.nc")
      mask_fil = os.path.join(run_mask,case+".fx.sftlf.nc")
      case_dict[case] = Case(data_fil, var= "psl", color="blue", label=labels[i])
      case_dict['mask'] = Case(mask_fil, var= "sftlf", color="blue", label=labels[i])
