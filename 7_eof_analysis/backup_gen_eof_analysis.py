...
def draw_eof_map(da,season,region,regnam,mip,exp,relm,case,case_id,period,out_path,fig_path):
  ...
  for typ in [ "raw", "detrend"]:
    ...
    for ieof in range(neofsub):
      ...
      # Show the plot
      ax1, fill1 = draw_regression_map(lons,lats,vcor,vreg,pval,fig,region,
              fontsize,'EOF{} on {}'.format(ieof+1,vstr),grid[0:2,ieof],vmin,vmax,nlev)
      axs.append(ax1)
      fills.append(fill1)
      del(ax1,fill1)
...
def draw_regression_map(lats,lons,cor,reg,pval,fig,region,font,title,grid_space,vmin,vmax,nlev):
  sig    = pval
  sig[:] = 1.0 - sig[:]
  t90    = 0.94
  t95    = 0.95
  ...
  if use_plotstyle == 'contour' or use_plotstyle == 'contourf':
      x, y = transform_to_proj(reg.longitude, reg.latitude)
 ...
 #ax.background_img(name='BlueMarbleBright', resolution='high')
  ...
  gl.ylocator = mticker.FixedLocator(np.arange(-90,0,10))
  gl.yformatter = LATITUDE_FORMATTER
  gl.xlabel_style = dict(weight='normal', color='black')
  gl.ylabel_style = dict(weight='normal', color='black')
  ...
  fillplot = ax.contourf(x,y,reg, **kwargs)
